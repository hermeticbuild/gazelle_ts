...
func TestApplyDirective_Strings(t *testing.T) {
	...
	applyDirective(cfg, rule.Directive{Key: directiveVisualModuleName, Value: "visuals"})
	...
	if cfg.visualModuleName != "visuals" {
		t.Errorf("visualModuleName = %q", cfg.visualModuleName)
	}
...
func TestApplyDirective_AppendDirectives(t *testing.T) {
	...
	applyDirective(cfg, rule.Directive{Key: directiveVisualModulePattern, Value: "*.stories.tsx"})
	...
	if !contains(cfg.visualModulePatterns, "*.stories.tsx") {
		t.Errorf("visualModulePatterns missing *.stories.tsx: %v", cfg.visualModulePatterns)
	}
	...
	count = 0
	for _, p := range cfg.visualModulePatterns {
		if p == "*.stories.tsx" {
			count++
		}
	}
...
func TestClone_Independent(t *testing.T) {
	...
	parent.visualModulePatterns = append(parent.visualModulePatterns, "*.stories.tsx")
	...
	child.visualModulePatterns = append(child.visualModulePatterns, "**/*.storybook.tsx")
	...
	if contains(parent.visualModulePatterns, "**/*.storybook.tsx") {
		t.Errorf("parent visualModulePatterns mutated: %v", parent.visualModulePatterns)
	}
